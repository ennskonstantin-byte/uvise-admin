...
// Kern-Inhalt wie app/dashboard/page.tsx (Erinnerungen, Mitarbeiter-Suche/
// -Grid, Mitarbeiter einladen/Unterweisung erstellen), aber ohne die
// Chef-only-Abschnitte Abo/Testphase, Social-Übersicht, Feedback-Karte --
...
export default function SifaDashboardPage() {
  ...
  const reminders = [
    ...
    ...trainings
      ...
      .map((t) => ({
        ...
        sub: t.status === "abgelaufen" ? "Jährliche Kontrolle überfällig — bitte prüfen" : "läuft ab",
  ...
  return (
    <>
      ...
      <PageHeader
        title={company?.name ?? "Firmen-Dashboard"}
        subtitle="Du arbeitest als SiFa in dieser Firma"
        action={
            ...
            {TABS.map((c) => (
              <button
                ...
                className={`min-h-11 rounded-full px-4 py-2 text-sm transition-colors ${
                  category === c ? "text-white" : "border border-border text-foreground/70 hover:border-foreground/30"
                }`}
                style={category === c ? { background: "var(--accent-gradient)" } : undefined}
              >
                {c}
              </button>
            ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((employee) => (
            <EmployeeCard
              key={employee.id}
              employee={employee}
              hrefBase={`/sifa/${params.companyId}/mitarbeiter`}
            />
          ))}
        </div>

        {filtered.length === 0 && (
          <p className="text-foreground/65 text-sm mt-10 text-center">Keine Mitarbeiter gefunden.</p>
        )}
      </Card>

      {showEmployeeWizard && <NewEmployeeWizard onClose={() => setShowEmployeeWizard(false)} />}
