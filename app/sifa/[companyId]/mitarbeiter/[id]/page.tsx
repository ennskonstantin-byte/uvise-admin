...
// Mitarbeiter-Detail (SiFa) -- Web-Port von Phase 5/App, identischer
// Kern-Inhalt zu app/mitarbeiter/[id]/page.tsx, nur der "Zurück"-Link
// zeigt auf die firmen-gescopte Mitarbeiter-Liste.
...
function trainingRowStatus(
  ...
  return overdue ? { label: "Überfällig", color: "var(--ampel-red)" } : { label: "Ausstehend", color: "#f59e0b" };
}
...
export default function SifaEmployeeDetailPage() {
  ...
  const listHref = `/sifa/${params.companyId}/mitarbeiter`;
  ...
  if (!employee) {
    return (
      <Card>
        ...
        <Link href={listHref} className="text-sm underline mt-2 inline-block">
          Zurück zur Liste
        </Link>
      </Card>
    );
  ...
  return (
    <>
      <Link href={listHref} className="inline-flex items-center gap-2 text-sm text-foreground/60 hover:text-foreground mb-6">
        <ArrowLeft size={16} />
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-4 mb-8">
          <EmployeeAvatar vorname={employee.vorname} nachname={employee.nachname} fotoUrl={employee.fotoUrl} size={64} />
        ...
        <div className="rounded-3xl border border-border divide-y divide-border overflow-hidden mb-8">
              ...
              <div key={et.id} className="flex items-center gap-4 px-5 py-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-foreground/65 flex items-center gap-1.5">
                    <span className="inline-block h-2 w-2 rounded-full shrink-0" style={{ background: rowStatus.color }} />
                    {rowStatus.label}
          ...
          {empTrainings.length === 0 && (
            <p className="px-5 py-4 text-sm text-foreground/65">Keine Unterweisungen zugewiesen.</p>
          )}
        </div>
        ...
        <div className="rounded-3xl border border-border divide-y divide-border overflow-hidden">
          {empQualifications.map((q) => (
            ...
            >
              ...
              <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{ background: QUALIFICATION_STATUS_COLOR[q.status] }} />
            </button>
          ))}
          {empQualifications.length === 0 && (
            <p className="px-5 py-4 text-sm text-foreground/65">Keine Qualifikationen erfasst.</p>
          )}
        </div>
      </Card>

      {editing && <EditEmployeeModal employee={employee} onClose={() => setEditing(false)} />}
      {assigning && <AssignTrainingToEmployeeModal employee={employee} onClose={() => setAssigning(false)} />}
      {addingQualification && (
